...
def get_data(pageNo,eID):  
    ...
    for d in soup.findAll('div', attrs={'class':'a-section a-spacing-none aok-relative'}):
        ...
        #Update numerical ID and status as active or inactive
        if eID is not None:
            all1.append(eID)
            all1.append(statusVar)
...
for i in range(1,no_pages+1):
    results.append(get_data(i,eID))
    eID += 50
#print(results)
flatten = lambda l: [item for sublist in l for item in sublist]
df = pd.DataFrame(flatten(results),columns=['entryID','status','Book_Name','Author','Rating','Customers_Rated', 'Price','Imageurl'])
#print(df)
